...
class SaleController extends Controller
{
    ...
    {      
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            return view('sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'search_value', 'status_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        }
        else
    ...
    {
        echo $request->input('search_value');
        $columns = array( 
            1 => 'created_at', 
            2 => 'reference_no',
            7 => 'grand_total',
        ...
        if(Auth::user()->role_id > 2 && $status_id != 2)
        ...
        elseif(Auth::user()->role_id > 2)
            $totalData = Sale::where('user_id', Auth::user()->id)
                        ->whereDate('created_at', '>=' ,$request->input('starting_date'))
                        ->whereDate('created_at', '<=' ,$request->input('ending_date'))
                        ->count();
        elseif($status_id != 2)
        ...
        $searching = $request->input('search_value');
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            if(($sale->is_valide == 1 && $sale->payment_status == 4) || Auth::user()->id == 1) {
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            $lims_cities_list = City::all();

            $lims_general_setting_data = GeneralSetting::latest()->first();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_pos_setting_data = PosSetting::latest()->first();

            return view('sale.create',compact('role', 'lims_cities_list', 'lims_general_setting_data', 'lims_tax_list', 'lims_pos_setting_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            if (Auth::user()->role_id == 1) {
                ...
                $lims_customer_list = Customer::where([['is_active', true],['created_by', Auth::user()->id]])->get();
            }
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_general_setting_data = GeneralSetting::latest()->first();
